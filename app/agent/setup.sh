...
create_script()
{
	...
	echo "image=\$registry/lovanow_beta/appagent:v1" >> $script
    ...
    echo "	docker run -d --rm --name \$docker_name --net=bridge -v /var/run/docker.sock:/var/run/docker.sock --env APP_PORT=8000 -p 8001:8000 \$image" >> $script
	echo "}" >> $script
